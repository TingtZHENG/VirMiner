...
while (<IN1>) {
	...
	$hash{$query_name}=$POG_subject_name;
	}
	

open (OUT,">$fOut") or die $!;
foreach my $gene_id (keys %hash) {
	my $subject_id=$hash{$gene_id};
	my @POG_id_complete_set=();
	if ($subject_id =~ /\[(.*)\]/)
	{
		my $POG_id_all=$1;
		if ($POG_id_all=~/,/){
			my @POG_id_info=split /,/,$POG_id_all;
			for (my $i=0;$i<@POG_id_info;$i++){
				push @POG_id_complete_set,$POG_id_info[$i];
			}
		}else{
				push @POG_id_complete_set,$POG_id_all;
				}
			}
	...
	chomp; 
	next if (/^$/);
	my($POG,$taxid,$description,$rank)= split /\t/,$_;
	for (my $j=0;$j<@POG_id_complete_set;$j++){
		if ($POG_id_complete_set[$j] eq $POG){
			print OUT "$gene_id\t$subject_id\t$POG\t$taxid\t$description\t$rank";
		    print OUT "\n";
		    }
	}
	}
